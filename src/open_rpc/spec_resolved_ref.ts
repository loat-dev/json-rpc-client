 ...
 */
type ExtractComponentNames<S, ComponentType extends string> = 
  S extends { components: infer C }
    ? C extends Record<ComponentType, infer Items>
 ...
 */
type ValidRefForComponent<S, ComponentType extends string> = 
  `#/components/${ComponentType}/${ExtractComponentNames<S, ComponentType>}`;

/**
 * All valid $ref strings for a schema
 */
type ValidRef<S> = 
  | ValidRefForComponent<S, "schemas">
  | ValidRefForComponent<S, "contentDescriptors">
  | ValidRefForComponent<S, "errors">
  | ValidRefForComponent<S, "examples">
  | ValidRefForComponent<S, "examplePairings">
  | ValidRefForComponent<S, "links">
  | ValidRefForComponent<S, "tags">;

/**
 * Check if a $ref is valid
 */
type IsValidRef<S, Ref extends string> = Ref extends ValidRef<S> ? true : false;

/**
 * Recursively collect all invalid $refs from a type
 */
type CollectInvalidRefs<S, T> = 
  T extends { $ref: infer Ref }
    ? Ref extends string
      ? IsValidRef<S, Ref> extends false
        ? Ref
        : never
      : never
    : T extends readonly (infer Item)[]
    ? CollectInvalidRefs<S, Item>
    : T extends object
    ? { [K in keyof T]: CollectInvalidRefs<S, T[K]> }[keyof T]
    : never;

/**
 * Check if schema has any invalid refs
 */
type HasInvalidRefs<S> = [CollectInvalidRefs<S, S>] extends [never] ? false : true;

/**
 * Validated OpenRPC Document type - compilation fails if invalid $refs exist
 */
export type ValidatedOpenRPCDocument<S> = 
  HasInvalidRefs<S> extends true
    ? { 
        __VALIDATION_ERROR__: `Schema contains invalid $ref(s)`;
        __INVALID_REFS__: CollectInvalidRefs<S, S>;
      }
    : S;
 ...
 */
export function defineOpenRPCDocument<const Schema extends OpenRPCDocument>(
